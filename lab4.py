class Car:
    ...
    def is_doors_closed(self) -> bool:
        """
        Метод, проверяющий, закрыты ли двери
...
if __name__ == "__main__":
    ...
    class Truck(Car):
        ...
        def is_doors_closed(self) -> bool:
            ...
            """
            Метод для установки приложения необходимо перегрузить(так как может меняться
        ...
        def open_door(self, door_number) -> None:
            """
            Метод, открывающий дверь(метод необходимо перегрузить, так как может меняться
            количество дверей в автомобиле)
            :param door_number: номер двери, которую необходимо открыть
            """
            ...
